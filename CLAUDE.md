...
`fixtures_contract.rs`) and the catalog conformance suite (`catalog_conformance.rs`
over the vendored pinned set in `tests/fixtures/catalog-conformance/v1/`).
...
- **Область ревью: инструкции агентов — код, проза и планы — нет.** Правило
  области кладёт под прозу весь Markdown, но `CLAUDE.md`, `AGENTS.md` и всё
  дерево `.claude/*` прозой не являются: это инструкции, определяющие
  поведение агентов в этом репозитории (границы полирепо, git-workflow,
  правила мержа, навыки). Дефект в них **исполняется**, а не читается, —
  ради этого класса в правиле и заведён `CODE_OVERRIDE` (в его шапке довод
  опирается на steward#170, где оба блокирующих gold-дефекта лежали в таком
  Markdown). Сознательно **остаются прозой** и ревьюятся человеком:
  `TODO.md`, `docs/*`, `arbiter-spec.md`, `README.md`, `SUGGESTIONS.md` и
  планы в корне — ради них разделение и вводилось.
  До 2026-09-21 это держал repo-конфиг `.github/codex/review-scope.env`,
  заведённый как временная мера; правило приехало в SSOT волной devtools#292,
  конфиг снят как дубль, а довод сохранён здесь. Проверять сдвиг покрытия —
  `sh scripts/review/local.sh --base <base> --head <ветка> --fingerprint-only`:
  код 5 = «всё отфильтровано как проза», код 0 = уходит ревьюеру.
