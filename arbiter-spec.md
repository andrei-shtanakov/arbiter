...
CREATE TABLE IF NOT EXISTS agents (
    id                TEXT PRIMARY KEY,           -- agent_id; usually "<harness>@<model>" (e.g. "claude_code@claude-sonnet-4-6", "codex_cli@gpt-5.5"), harness-only allowed (e.g. "aider")
...
**Request schema:** see `arbiter-mcp/tests/contract/report_benchmark-v1.schema.json` (mirror of Maestro's `contracts/benchmark/`). Required: `payload_version="1.0.0"`, `run_id`, `benchmark_id`, `agent_id`, `ts` (RFC3339), `score`, `score_components`, `duration_seconds`, `per_task`, `per_task_total_count`, `per_task_truncated`.
...
class FallbackScheduler:
    ...
    AGENT_ORDER = ["claude_code@claude-sonnet-4-6", "codex_cli@gpt-5.5", "aider"]
...
> `claude_code@claude-sonnet-4-6`, `codex_cli` → `codex_cli@gpt-5.5`). The
