...
- Незнакомое значение enum (`status`, `kind`) — **degrade-with-warning**, не
  ...
  информационен и ни на что не влияет. **Гарантия «консервативно» держится на
  allowlist-проверке у потребителя** (`status == Active`), не на denylist
  (`status != Retired`): будущая runtime-интеграция обязана проверять позитивно,
  иначе `Other`-модель (напр. `preview`) утечёт в роутинг.
