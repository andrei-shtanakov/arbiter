...
- [ ] review-kit-catchup-scope @owner:github:andrei-shtanakov @id:review-kit-catchup-scope — принят inbox issue #107 (from steward, срез B области ревью, steward#172). Догнать вендор-копию кита до текущего релиза: `scripts/review/prose-paths.env` (пиненая копия SSOT devtools, приезжает через steward) + фильтр области в `local.sh` + новый код выхода 5 «всё отфильтровано». Первый релиз (харнесс-слой `harness-claude`) уже завендорен — #105/#106, `steward @ a2d7e71`; премиса issue про «шесть членов без harness-claude» была снята до приёмки. Волна по флоту (steward-план Task 6) этот репо не касается: идём отдельной работой. Двухфазность обязательна — джоба `review-kit-integrity` исполняет `checksum.sh` из base, поэтому PR-1 несёт только инвентарь (`?scripts/review/prose-paths.env`) и свою строку PIN, PR-2 — файл правила, `local.sh`, остальные члены и строки PIN. `.github/hooks/pre-push` в этом репо нет — шаг волны про хук неприменим
