...
### Ре-вендор review-kit до релиза «область ревью» (inbox #107)

- [x] review-kit-catchup-scope @owner:github:andrei-shtanakov @id:review-kit-catchup-scope — сделано (inbox issue #107, from steward; PR #109 + #110): вендор-копия кита догнана до `steward @ c18bf87` — релиз «область ревью» (срез B, steward#172). Прозаический диапазон больше не доходит до модели: код выхода **5** «всё отфильтровано», отличный от 0 («пуст сам диапазон») — слить их значило бы отдать обвязке approve, которого модель не выносила; смешанный диапазон уходит одной кодовой частью. Правило — `scripts/review/prose-paths.env`, восьмой член кита. Двухфазно, иначе чекер из base отказал бы на PIN со членом вне состава: #109 — инвентарь, #110 — файл и остальные члены.
  - Сверх запроса: поведенческий Markdown возвращён под ревью (`.github/codex/review-scope.env`, `PROSE_REVIEW=paths`) — `CLAUDE.md`/`AGENTS.md`/`.claude/*` это инструкции агента, дефект в них исполняется, а не читается; правило кита положило бы их под `*.md` молча. `TODO.md`, `docs/*`, `arbiter-spec.md` и планы отфильтрованы сознательно
  - Починена drift-вахта: список членов перечислял шесть файлов и был слеп к `harness-claude` с #106 — адаптер не сверялся с апстримом ни разу
  - Премиса issue была снята до приёмки: первый из двух релизов (харнесс-слой) уже приехал с #105/#106, отставание было только на срез B
  - Две находки ревью уехали в апстрим байт-в-байт копии: steward#173 (шапка инвентаря противоречит себе) и steward#175 (режимы-запросы инвалидируют sidecar-ы — вопросом)
  - `.github/hooks/pre-push` в репо нет — шаг рецепта про хук неприменим

### Allow/deny для деплой-действий (inbox #104, from deployer)

- [ ] deploy-action-decision-tool @owner:github:andrei-shtanakov @trigger:"deployer сообщает, что у него появилось мутирующее действие (проснулся todo://deployer/arbiter-policy-gate-seam)" @id:deploy-action-decision-tool @epic:eco.governance-plane — принято 2026-09-22 (inbox issue #104, from deployer). Премиса issue подтверждена: решения «можно ли выполнить это деплой-действие» нет ни в одном из шести MCP-инструментов, ни в `arbiter-cli`/`arbiter-core`. Единственная authority-поверхность — RD-006 (`arbiter_core::authority::check_authority`) — отвечает на другой вопрос: «какие *агенты* допущены в role×phase» на route-time, и её дизайн (`docs/2026-07-12-authority-split-design.md` §1) сознательно вынес runtime admission в v2+. Гейтить пока нечего (L2 деплоера — песочница без мутаций), поэтому это пункт-ожидание с триггером на событие **их** стороны: до сигнала arbiter ничего не строит — тул без потребителя был бы контрактом, угаданным за него. Форма, когда триггер сработает: седьмой MCP-tool (bump `protocolVersion`, замороженный DTO `route_task` не трогается), плоскость MAY, не CAN, по паттерну RD-006 — SSOT политики у steward, пиненая копия в `config/`, fail-closed, отказ — first-class audited decision с `policy_sha`. Человеческий аппрув на проде — вне arbiter: arbiter отдаёт allow / deny / needs-approval, аппрув собирает вызывающая сторона. От deployer при сигнале нужны словарь действий (что именно гейтится) и контекст решения (target / env / actor) — без них схему не резать
  - Сознательно не `@blocked_by`: пункт deployer триггерится их же событием и сам зависит от нашего тула — блокировка была бы круговой (симметрично их формулировке)
  - Это первое исключение из «6 инструментов заморожены» в «Не делаем»: оговорка там была «без запроса потребителя», запрос теперь есть
...
- ❌ Дальнейшее расширение MCP API без запроса потребителя — 6 инструментов заморожены (первый запрос потребителя — inbox #104, пункт `deploy-action-decision-tool`: расширение по нему допустимо, но только после его триггера)
