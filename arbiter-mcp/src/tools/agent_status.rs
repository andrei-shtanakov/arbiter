...
mod tests {
    ...
    fn all_agents_returned_without_params() {
        ...
        assert!(ids.contains(&"claude_code@claude-sonnet-4-6"));
        assert!(ids.contains(&"codex_cli@gpt-5.5"));
    ...
    fn single_agent_returned_with_agent_id() {
        ...
        let args = serde_json::json!({ "agent_id": "claude_code@claude-sonnet-4-6" });
        let result = execute(&args, &db, &registry, 5).unwrap();

        assert_eq!(result.agents.len(), 1);
        assert_eq!(result.agents[0].id, "claude_code@claude-sonnet-4-6");
    ...
    fn stats_reflect_outcomes_with_by_language_and_by_type() {
        ...
        // Record outcomes for claude_code@claude-sonnet-4-6
    ...
    fn result_to_json_structure() {
        let result = StatusResult {
            agents: vec![AgentStatus {
                id: "claude_code@claude-sonnet-4-6".to_string(),
                display_name: "Claude Code".to_string(),
        ...
        let a = &agents[0];
        assert_eq!(a["id"], "claude_code@claude-sonnet-4-6");
