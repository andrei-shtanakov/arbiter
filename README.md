...
The server reads JSON-RPC 2.0 from stdin and writes responses to stdout. Logs are structured OTel JSONL written to a file per process — `$ORCHESTRA_LOG_DIR/arbiter-<pid>.jsonl`, defaulting to `./logs/<pipeline_id>/` (observability v1 contract). stderr is used only for fatal pre-init errors and as a fallback when log initialization fails.
